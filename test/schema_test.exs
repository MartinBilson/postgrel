defmodule SchemaTest do
  ...
  test "decode hstore inside a schema", context do
    ...
    assert [[%{"Bubbles" => "7", "Name" => "Frank"}]] =
             query(~s{SELECT '"Name" => "Frank", "Bubbles" => "7"'::test.hstore}, [])

    assert [[%{"non_existant" => nil, "present" => "&accounted_for"}]] =
             query(
               ~s{SELECT '"non_existant" => NULL, "present" => "&accounted_for"'::test.hstore},
               []
             )

    assert [[%{"spaces in the key" => "are easy!", "floats too" => "66.6"}]] =
             query(
               ~s{SELECT '"spaces in the key" => "are easy!", "floats too" => "66.6"'::test.hstore},
               []
             )

    assert [[%{"this is true" => "true", "though not this" => "false"}]] =
             query(
               ~s{SELECT '"this is true" => "true", "though not this" => "false"'::test.hstore},
               []
             )
  end
end
